...
$('api').addEventListener('input',()=>{connectionAttempt++;catalogBase=null;invalidate();$('connection-state').textContent='Alamat berubah. Klik Periksa koneksi untuk memuat katalog backend ini.';});
...
form.addEventListener('submit',async e=>{
 e.preventDefault();if(!form.reportValidity()||busy)return;let base;try{base=endpoint();}catch{document.querySelector('.connection').open=true;$('api').focus();status('Isi alamat backend pada Koneksi simulator terlebih dahulu.',true);return;}
 ...
 finally{busy=false;const d=read(),g=geometry(d);$('run').disabled=!form.checkValidity()||!catalog?.modules.some(x=>x.name===d.module)||!catalog?.inverters.some(x=>x.name===d.inverter)||!g.modules_per_face||g.modules_per_face*g.face_ids.length>3000;}
});
