...
function update(){
 invalidate();layoutReady=false;resolvedSizing=null;layoutVersion++;clearTimeout(layoutTimer);
 field('modules_per_string').readOnly=field('sizing_mode').value!=='manual';field('inverters_per_face').readOnly=field('sizing_mode').value==='maximize';
...
async function refreshLayout(version){
 try{
  ...
  const resolved={...d,modules_per_string:p.modules_per_string,inverters_per_face:p.inverters_per_face};
  ...
  const count=p.modules_per_face*faces,dc=count*m.power_w/1000,ac=p.inverters_per_face*faces*i.power_w/1000;
  ...
  $('layout-info').textContent=`${p.positions_per_face*faces} posisi atap · ${count} panel terpasang · ${p.unused_per_face*faces} posisi tersisa. ${p.modules_per_string} modul/string · ${p.strings_per_face} string/bidang · ${p.inverters_per_face} inverter/bidang. Pembatas: ${p.reason}.`;
  $('sizing-info').textContent=`Rentang string: ${p.string_min}–${p.string_max} modul. ${p.validation==='incomplete'?'Validasi listrik belum lengkap: '+p.missing_fields.join(', '):'Pemeriksaan listrik awal lulus untuk data yang dimasukkan; bukan verifikasi desain final.'} Distribusi string per inverter: `+p.groups.map(x=>`${x.inverter_count} unit × ${x.strings_per_inverter} string`+(x.strings_per_mppt?' (MPPT: '+x.strings_per_mppt.join('/')+')':'')).join('; ');
...
$('api').addEventListener('input',()=>{connectionAttempt++;catalogBase=null;layoutReady=false;resolvedSizing=null;layoutVersion++;$('run').disabled=true;invalidate();$('connection-state').textContent='Alamat berubah. Klik Periksa koneksi untuk memuat katalog backend ini.';});
